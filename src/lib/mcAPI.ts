...
const MCUsername = z.string().min(3).max(16).regex(/^\w+$/);
...
export async function getMCName(mcName: string): Promise<string | undefined> {
	return (await getMCProfile(mcName))?.id;
}
