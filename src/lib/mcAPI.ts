...
export async function getMCProfile(mcName: string): Promise<MCProfile | undefined> {
	try {
...
export const getMCName = async (mcName: string): Promise<string | undefined> => (await getMCProfile(mcName))?.id;
