...
export class UserEvent extends Listener<typeof Events.MessageCreate> {
	public override async run(message: Message<true>) {
		...
		if (message.guildId !== Servers.SkyClient)
			return;
...
function findLogs(txt: string) {
	...
	if (hastebinMatch)
		ret.push(`https://hst.sh/raw/${hastebinMatch[1]}`);

	const mclogsMatch = txt.match(/https:\/\/mclo\.gs\/([a-z0-9]*)/i);
	if (mclogsMatch)
		ret.push(`https://api.mclo.gs/1/raw/${mclogsMatch[1]}`);
...
async function verbalizeCrash(log: string, isSkyclient?: boolean) {
	...
	const relevantInfo = crashData.fixes.filter(fix =>
		fix.causes.every((type) => {
			if (type.method === 'contains')
				return log.includes(type.value);
			else if (type.method === 'regex')
				return log.match(new RegExp(type.value));
			else if (type.method === 'contains_not')
				return !log.includes(type.value);
			else return false;
		}),
	);
