...
export class MessageListener extends Listener<typeof Events.MessageCreate> {
	...
	public override async run(message: Message) {
		...
		if (channel.id === Channels.Trolling)
			return; // exclude trolling
		if (member.roles.cache.has(Roles.CoolPeople))
			return; // cool people
		if (member.roles.cache.has(Roles.NoGiveaways))
			return; // already has no giveaways
