...
async function check(id: string): Promise<MessageReplyOptions | undefined> {
	...
	// TODO: Typings
	const averagePerHour = data.reduce((a: any, b: { count: any }) => a + b.count, 0) / 24;
	...
	const getMessages = (hour: number) =>
		// TODO: Typings
		data.filter((group: { date_part: number }) => group.date_part == hour).reduce((a: any, b: { count: any }) => a + b.count, 0);
	...
	if (currentHourMessages < silentThreshold) {
		...
		const percent = ((currentHourMessages / (averagePerHour * 24)) * 100).toFixed(1);
